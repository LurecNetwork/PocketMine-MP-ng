...
START_TIME_TO_CALCULATE=$(date +%s%N)
if [ -d "bin" ]; then
...
END_TIME_TO_CALCULATE=$(date +%s%N)
TIME_TAKE_TO_CALCULATE=$(echo "scale=3; ($END_TIME_TO_CALCULATE - $START_TIME_TO_CALCULATE) / 1000000000" | bc)
